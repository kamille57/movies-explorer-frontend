...
function SearchForm({ cards, handleSearch, isSaved }) { 
  const [searchQuery, setSearchQuery] = useState(() => 
    localStorage.getItem(isSaved ? 'savedMoviesSearchQuery' : 'moviesSearchQuery') || ""
  ); 
  const [onlyShortMovies, setOnlyShortMovies] = useState(
    localStorage.getItem(isSaved ? 'savedOnlyShortMovies' : 'moviesOnlyShortMovies') === 'true' || false
  ); 
  ...
  const handleFilteredResults = (searchQuery) => {
    ...
    localStorage.setItem(isSaved ? 'savedMoviesSearchQuery' : 'moviesSearchQuery', searchQuery);
  }
  ...
  return (
    <section className="search">
      ...
      <FilterCheckbox setOnlyShortMovies={setOnlyShortMovies} onlyShortMovies={onlyShortMovies} />
    </section>
  );
}
